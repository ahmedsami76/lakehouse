...
3. build the docker image (e.g. `$ docker build -t spark .`)
4. start a container (e.g. `$ docker container run -it -v $(pwd)/data:/data -v $(pwd)/code:/code --name "sparkc" -h sparkc spark`)
